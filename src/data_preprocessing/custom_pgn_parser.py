...
GAME_REGEX = r'\[Event.*[^"](1/2-1/2|1-0|0-1)[^"]'

def custom_pgn_parser(stream: str) -> dict:

    game = {}
    game_re = re.compile(GAME_REGEX, flags=re.DOTALL)

    game_raw = game_re.search(stream)
    game_raw = stream[game_raw.span()[0] : game_raw.span()[1]]
    ...
    for h in HEADER_REGEX_dict:
        ...
        game[h] = header_match[0] if len(header_match) > 0 else '?' 
